...
class LSTM_Model():
    ...
    def calculate_technical_indicators(self, data):
        df = pd.DataFrame(self.y, columns=['close'])
    ...
    def data_preprocess(self, dataset, iStart, iEnd, sHistory, forward_look=1):
        ...
        for i in range(iStart, iEnd):
            ...
            reshape_entity = np.append(reshape_entity, dataset[indices])
            
            # Kiểm tra kích thước của reshape_entity trước khi reshape
            if reshape_entity.size == sHistory:
                self.data.append(np.reshape(reshape_entity, (sHistory, 1)))
            else:
                print(f"Warning: reshape_entity size {reshape_entity.size} does not match sHistory {sHistory}")
    ...
    def prepare_test_train(self):
        ...
        tech_indicators = self.calculate_technical_indicators(self.y)
        
        # Chuẩn hóa dữ liệu
        scaler = MinMaxScaler()
        normalized_data = scaler.fit_transform(tech_indicators[['close', 'RSI', 'MACD', 'Signal_Line', 'Upper_Band', 'Lower_Band', 'Momentum']])
        
        # Tạo chuỗi thời gian X và y
        data, target = self.data_preprocess(normalized_data, 0, int(len(y) * self.train_test_split), self.past_history, forward_look=self.forward_look)
        self.xtrain.append(data)
        self.ytrain.append(target)
        
        data, target = self.data_preprocess(normalized_data, int(len(y) * self.train_test_split), None, self.past_history, forward_look=self.forward_look)
    ...
    def model_LSTM(self):
        ...
        input_shape = (self.past_history, 7)  # 7 là số lượng chỉ báo kỹ thuật
        
        if self.naive:
            self.model.add(tf.keras.layers.LSTM(64, input_shape=input_shape))
        else:
            self.model.add(tf.keras.layers.LSTM(64, return_sequences=True, input_shape=input_shape))
        
        for i in range(self.depth):
            self.model.add(tf.keras.layers.LSTM(32, return_sequences=True))
